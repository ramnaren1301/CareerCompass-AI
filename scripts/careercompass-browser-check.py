#!/usr/bin/env python3
"""Browser regression for CareerCompass AI v3.0 native-ready, selection-first degree-to-career guide."""
...
RESULT_PATH = ROOT / "docs" / "careercompass-e2e-results.json"
...
def build_inline_document() -> str:
    ...
    catalog = (BUILD_ROOT / "data" / "careercompass-career-catalog.json").read_bytes()
    ...
    for path in sorted((BUILD_ROOT / "src").glob("*.js")):
        ...
        source = re.sub(
            r'(from\s+["\'])\./([^"\']+)(["\'])',
            lambda match: f'{match.group(1)}@careercompass/{Path(match.group(2)).stem}{match.group(3)}',
            source,
        )
        source = re.sub(
            r'(import\s*["\'])\./([^"\']+)(["\'])',
        ...
        module_map[f"@careercompass/{path.stem}"] = (
    ...
    return (
        ...
        '<title>CareerCompass AI — Degree-to-Career Guide</title><style>' + styles + '</style>'
        '<script type="importmap">' + imports + '</script></head><body>'
        '<div id="app" class="app-shell"></div>'
        '<script type="module">import "@careercompass/app";</script></body></html>'
    )
...
def create_page(context: BrowserContext, errors: list[dict[str, str]]) -> Page:
    ...
    page.wait_for_function("window.CareerCompassAI?.tools?.length === 33", timeout=30_000)
    return page


def journey(page: Page) -> dict[str, Any]:
    return page.evaluate("window.CareerCompassAI.getState()")


def data_state(page: Page) -> dict[str, Any]:
    return page.evaluate("window.CareerCompassAI.getDataState()")


def wait_step(page: Page, step: str) -> None:
    page.wait_for_function(
        f"window.CareerCompassAI.getState().currentStep === {json.dumps(step)} && "
        "window.CareerCompassAI.getState().busy === false",
        timeout=30_000,
    )
...
def click_and_wait(page: Page, selector: str, step: str | None = None) -> None:
    ...
    if step:
        ...
        page.wait_for_function("window.CareerCompassAI.getState().busy === false", timeout=30_000)
...
def verify_no_horizontal_overflow(page: Page, label: str) -> None:
    metrics = page.evaluate(
        ...
        "app: document.querySelector('.compass-app')?.scrollWidth || 0 })"
    )
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        page.wait_for_function("window.CareerCompassAI.getState().directionMode === 'all'")
        ...
        page.wait_for_function("window.CareerCompassAI.getState().directionMode === 'groups'")
        ...
        page.wait_for_function("window.CareerCompassAI.getState().directionMode === 'fields'")
        ...
        page.wait_for_function("window.CareerCompassAI.getState().planApprovalId.length > 0")
        ...
        page.wait_for_function("window.CareerCompassAI.getState().selectedSkill.length > 0")
        ...
        page.wait_for_function(
            "window.CareerCompassAI.getState().experienceChoices.length > 0 && "
            "window.CareerCompassAI.getState().busy === false",
            timeout=30_000,
        )
        ...
        page.wait_for_function("window.CareerCompassAI.getState().selectedExperience !== null")
        ...
        mobile_page.wait_for_function("window.CareerCompassAI.getState().directionMode === 'fields'")
